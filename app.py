...
# --- 1. የ GITHUB ምልክት መደበቂያ (ይህ ክፍል ለሌሎች ተጠቃሚዎች የ GitHub icon ይደብቃል) ---
hide_github_only = """
    <style>
    /* የ GitHub iconን እና የ Streamlit menuን ለመደበቅ */
...
texts = {
    'EN': {
        ...
        'analysis_desc': "Feature Importance shows which factors (like grades or absences) influenced the AI result.",
    ...
    'AM': {
        ...
        'analysis_desc': "ይህ ትንታኔ የትኛው መረጃ በ AI ውሳኔ ላይ ከፍተኛ ተጽዕኖ እንዳሳደረ ያሳያል።",
...
# --- 7. LOAD ASSETS (አስተማማኝ የፋይል መንገዶች) ---
@st.cache_resource
def load_assets():
    try:
        # ፋይሎቹ ያሉት በዋናው ፎልደር ውስጥ ከሆነ (student_model.pkl እንጂ models/ ውስጥ ካልሆነ)
        scaler = joblib.load('models/scaler.pkl') # models/ ውስጥ ካለ እንዳለ ይቆይ
        rf_model = joblib.load('student_model.pkl') # ስሙን አስተካክዬዋለሁ
        return scaler, rf_model
    except:
        return None, None

scaler, rf_model = load_assets()
...
if selected == L['nav_home']:
    ...
    if rf_model and scaler:
        # የመረጃ ዝግጅት
        input_data = np.array([[age, g1, g2, failures, studytime, absences, 1 if L['male'] in sex else 0, 1, 1]])
        ...
        with col_l:
            ...
            if prediction == 0: st.error(L['desc_at_risk'])
            elif prediction == 1: 
    ...
    st.info(L['analysis_desc'])

elif selected == L['nav_info']:
    st.title("ℹ️ Resources")
...
# --- FOOTER ---
st.markdown(f"<br><hr><center>EduPredict AI | Mekdela Amba University | © 2026</center>", unsafe_allow_html=True)